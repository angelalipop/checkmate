...
class ServerOpenCVOMRProcessor {
  ...
  static OpenCVOMRResult process(
    ...
    try {
      ...
      for (final section in sections) {
        ...
        // Find the first physical bubble row in this section and align Q1 to it.
        // We deliberately choose the TOPMOST valid row instead of the nearest
        // repeating row, so Q1 cannot be remapped to Q2.
        final sectionTranslation = _findTopmostBubbleRowTranslation(
          bubbleCandidates,
        ...
        for (int i = 0; i < questions.length; i++) {
          ...
          for (
            ...
            final logicalCenterY =
                rowTop + (_bubbleSize / 2.0) + sectionTranslation.dyLogical;
            final pixelCenterX =
                ((logicalCenterX + sectionTranslation.dxLogical) * scaleX)
                    .round();
  ...
  static _SectionTranslation _findTopmostBubbleRowTranslation(
    ...
    for (
      ...
      // Once the first valid physical row has been found, only inspect one
      // tolerance-band around that row. This keeps Q1 anchored to the TOPMOST
      // real row instead of drifting to Q2.
      if (firstValidDy != null &&
          dy > firstValidDy + (_topRowYToleranceLogical * 2.2)) {
        break;
      }
      ...
      for (
        ...
        final candidateScore = _TopRowTranslationScore(
          dxLogical: dx,
          dyLogical: dy,
          matches: matches,
          quality: quality,
        ...
        if (bestAtThisY == null ||
            _isBetterTopRowTranslation(candidateScore, bestAtThisY)) {
          bestAtThisY = candidateScore;
        }
      }

      if (bestAtThisY == null || bestAtThisY.matches < requiredMatches) {
        continue;
      }
      ...
      if (bestInTopmostBand == null ||
          _isBetterTopRowTranslation(bestAtThisY, bestInTopmostBand)) {
        bestInTopmostBand = bestAtThisY;
      }
    }
    ...
    return _SectionTranslation(
      dxLogical: bestInTopmostBand.dxLogical,
      dyLogical: bestInTopmostBand.dyLogical,
    );
  }

  static bool _isBetterTopRowTranslation(
    _TopRowTranslationScore candidate,
    _TopRowTranslationScore current,
  ) {
    if (candidate.matches != current.matches) {
      return candidate.matches > current.matches;
    }

    if ((candidate.quality - current.quality).abs() > 0.000001) {
      return candidate.quality > current.quality;
    }

    // Tie-break toward the smallest correction.
    final candidateMovement =
        candidate.dxLogical.abs() + candidate.dyLogical.abs();
    final currentMovement = current.dxLogical.abs() + current.dyLogical.abs();
